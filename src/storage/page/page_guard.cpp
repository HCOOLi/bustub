...
ReadPageGuard::ReadPageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
  ...
  is_valid_ = true;
  DiskRequest disk_request = DiskRequest{
    .is_write_ = false,
    .data_ = this->frame_->GetDataMut(),
    .page_id_ = this->page_id_,
    .callback_ = disk_scheduler_->CreatePromise(),
};
  auto future = disk_request.callback_.get_future();
  this->disk_scheduler_->Schedule(std::move(disk_request));
  fmt::println("future returned={} ",future.get());
...
void ReadPageGuard::Drop() {
  ...
  if (is_valid_) {
    auto count = --this->frame_->pin_count_;
    is_valid_ = false;
    if (count==0) {
      this->replacer_->SetEvictable(this->page_id_,true);
    }
...
auto WritePageGuard::operator=(WritePageGuard &&that) noexcept -> WritePageGuard & {
  if (is_valid_ ) {
    fmt::println("trying to overwrite a guard");
    this->Drop();
  }
  this->page_id_ = that.page_id_;
  this->frame_ = std::move(that.frame_);
